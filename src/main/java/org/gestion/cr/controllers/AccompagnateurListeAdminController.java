...
public class AccompagnateurListeAdminController implements
		HandlerExceptionResolver {
	private int page = 0;
	private int nbrLignesAccompagnateurs = 5;
	...
	public String validerModificationAccomp(@Valid Accompagnateur accomp,BindingResult bindingResult, Model model, MultipartFile file)throws IOException {
		...
		if (ref != null) {
			...
			if (file.isEmpty()) {
				...
				file.transferTo(new File(path + "/" + "ACCOMPAGNATEUR_"	+ accomp.getIdPerson() + "_" + file.getOriginalFilename()));
				accomp.setNomPhoto(path + "/" + "ACCOMPAGNATEUR_"+ accomp.getIdPerson() + "_" + file.getOriginalFilename());
			}
	...
	public String supprimerListeAccompagnateur(Long idAccompagnateur,Model model) {
	...
	public void chargerModel(Model model) {
		...
		model.addAttribute("accompagnateurs",metier.listAccompagnateurs(pos, getNbrLignesAccompagnateurs()));
	...
	public ModelAndView resolveException(HttpServletRequest request,
		...
		mv.addObject("accompagnateurs", metier.listAccompagnateurs(0, 5));
